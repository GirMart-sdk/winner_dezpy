...
app.post('/api/products', requireAuth, (req, res) => {
  ...
  if (!name || !price) {
    return res.status(400).json({ error: 'name y price son requeridos', success: false });
  }
  ...
  `, [
    ...
    if (sqliteErr) {
      ...
      return res.status(500).json({ error: sqliteErr.message, success: false });
    }
          ...
          if (stocksUpdated === stockEntries.length) {
            stmt.finalize((finalErr) => {
              if (finalErr) {
                ...
                return res.status(500).json({ error: finalErr.message, success: false });
              }
              res.json({ success: true, id: productId, message: 'Producto guardado exitosamente' });
            });
          }
        });
      });
...
app.post('/api/sales', (req, res, next) => {
  ...
  if (auth.startsWith('Bearer ')) {
    ...
    return res.status(401).json({ error: 'Autenticación requerida', success: false });
  }
});
...
function handlePostSales(req, res) {
  ...
  if (!total) return res.status(400).json({ error: 'total es requerido', success: false });
  ...
  `, [
    ...
    if (Array.isArray(items) && items.length > 0) {
      ...
      items.forEach(item => {
        stmt.run(saleId, item.name || item.product_name || '—', item.qty || 1, item.price || 0, item.size || 'U', (err) => {
          ...
          if (itemsProcessed === items.length) {
            stmt.finalize((err) => {
              ...
              res.json({ success: true, id: saleId, message: 'Venta registrada exitosamente' });
            });
          }
        });
      });
