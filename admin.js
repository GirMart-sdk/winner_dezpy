...
if (typeof window.API_KEY === "undefined") {
  window.API_KEY =
    localStorage.getItem("w_api_key") || "prod-api-key-winner-2026";
}
...
function selectPOSPaymentMethod(methodId) {
  ...
  document.getElementById("posPayTotal").textContent = admFmt(
    posPendingPayment.total,
  );
}
...
function renderPOSPaymentForm() {
  ...
  if (method.id === "efectivo") {
    ...
    document
      .getElementById("posPayCashReceived")
      .addEventListener("input", updatePOSCashChange);
...
function updatePOSCashChange() {
  const received =
    parseFloat(document.getElementById("posPayCashReceived").value) || 0;
  const total = posPendingPayment.total;
  const change = received - total;

  document.getElementById("posPayCashChange").textContent = admFmt(
    change >= 0 ? change : 0,
  );
}
...
function renderPOSSaleItems() {
  ...
  const discount =
    parseFloat(document.getElementById("posDiscount")?.value || 0) || 0;
  const subtotal = posCurrentSale.reduce(
  ...
  container.innerHTML = posCurrentSale
    .map(
      (item, i) => `
...
function posSearchProducts() {
  const search =
    document.getElementById("posSearch")?.value.toLowerCase() || "";
  ...
  let results = admInventory.filter(
    (p) =>
  ...
  if (results.length === 0) {
    container.innerHTML =
      '<div style="padding:16px;text-align:center;color:var(--gray-text)">Sin resultados</div>';
    return;
  }
...
function openQRScannerPOS() {
  ...
  if (video && typeof Html5Qrcode !== "undefined") {
    ...
    qrScanner.start(
      ...
      { fps: 10, qrbox: { width: 250, height: 250 } },
      (decodedText) => {
        const product = admInventory.find(
          (p) => p.id === decodedText || p.name === decodedText,
        );
        if (product) {
